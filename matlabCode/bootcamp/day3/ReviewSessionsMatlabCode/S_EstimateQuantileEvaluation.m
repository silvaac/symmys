...
Ge=G_Hat_e(i_T);        % sample median
Gb=G_Hat_b(i_T);        % sample mean estimator


%% replicability vs. "luck"

% functional of the distribution to be estimated
G_fX =QuantileMixture(0.5,a,m_Y,s_Y,m_Z,s_Z); % True value
...
% tentative estimator of unknown functional
Ge=G_Hat_e(I_T);        % sample median
Gb=G_Hat_b(I_T);        % sample mean
...
for j=1:length(m_s)
    ...
    Ge=G_Hat_e(I_T);        % tentative estimator of unknown functional: sample mean
    Gb=G_Hat_b(I_T);        % tentative estimator of unknown functional: sample median
