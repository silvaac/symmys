...
%weekly obs for the estimation
spot=spot(1:tau_tilde:end);
impVol=impVol(1:tau_tilde:end,:,:);
...
% log-changes in implied vol for different maturities
...
for i = 1:numCalls
    figure
    subplot(2,1,1)
    hist(LinearRets(:,i),NumBins)
    xlabel('linear return')
    subplot(2,1,2)
    plot(spot_,LinearRets(:,i),'.')
    xlabel('spot price')
    ylabel('linear return')
end
