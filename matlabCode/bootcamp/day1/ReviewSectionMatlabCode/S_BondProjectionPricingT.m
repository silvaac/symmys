...
Periods=tau/tau_tilde; % horizon-to-estimation ratio
u=TimesToMat-tau;

nu=8;
mus=0*u;
sigmas=(20+5/4*u)/10000;

Num_Scenarios=10^5;

%% projection and pricing 
BondCurrent_Prices_Shifted=exp(-FlatCurve*u);
...
figure(); set(gcf,'color','white','units', 'normalized', 'position',[.1 .1 .5 .75])
for n=1:N
    % project bond market to horizon
    [s,f_S,F_S]=ProjectionT(nu,mus(n),sigmas(n),Periods);
    
    % generate co-dependent changes in yield-to-maturity
    S(:,n) = interp1(F_S,s,U,'linear','extrap'); 

    % compute the horizon prices, (3.81) in "Risk and Asset Allocation" - Springer
    X=-u(n)*S(:,n);
    Z(:,n)=BondCurrent_Prices_Shifted(n)*exp(X); 
    
    % compute and plot linear returns
    L(:,n)=Z(:,n)/BondCurrent_Prices(n)-1;  
    
   
    subplot(N,1,n)
    hist(L(:,n),  round(10*log(Num_Scenarios)));
