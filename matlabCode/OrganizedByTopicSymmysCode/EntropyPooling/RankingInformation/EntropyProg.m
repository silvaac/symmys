...
options = optimset('GradObj','on','Hessian','on','HessFcn',@myHessian);
%options = optimset('GradObj','on');
%options = optimset('algorithm','trust-region-reflective','GradObj','on','Hessian','on','MaxIter',10000,'TolFun',1e-8,'MaxFunEvals',10000);
if ~K_
    v=fminunc(@nestedfunU,x0,options);
    %v=fminunc(@nestedfunU,x0);
    p_=exp(log(p)-1-Aeq_*v);
else
    lv=fmincon(@nestedfunC,x0,InqMat,InqVec,[],[],[],[],[],options);
    %lv=fmincon(@nestedfunC,x0,InqMat,InqVec,[],[],[],[],[]);
    l=lv(1:K_);
    v=lv(K_+1:end);
    ...
    function H = myHessian(lv,lambda)
